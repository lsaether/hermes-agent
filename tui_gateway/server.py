...
def _(rid, params: dict) -> dict:
    ...
    # If the resolved target is already live in-process, reuse it directly — no
    # DB row required (covers resume-by-title of a mobile-created live session).
    live = _live_session_for_target(str(target))
    if live is not None:
        sid, session = live
        _attach_transport_to_session(session)
    ...
    # Not live → rebuild from the DB. Validate the row and resolve any
    # continuation lineage (main's resume behaviour).
    found = db.get_session(target)
    if not found:
        return _err(rid, 4007, "session not found")
    try:
    ...
    try:
        ...
        if session:
            ...
            _emit(
                "session.info",
                params.get("session_id", ""),
                _session_info_for(agent, session.get("session_key")),
            )
