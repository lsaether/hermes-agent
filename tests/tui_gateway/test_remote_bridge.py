...
def test_session_resume_title_reuses_existing_live_session_and_attaches_current_transport(monkeypatch):
    ...
    class _FakeDB:
        ...
        def resolve_session_by_title(self, target):
            return "persisted-session" if target == "mobile-title" else None
...
def test_remote_bridge_allowlist_blocks_dangerous_methods_but_forwards_allowed(monkeypatch):
    """Only allowlisted methods reach dispatch over the bridge; shell.exec et al.
    are rejected with 4403 before dispatch. The stdio TUI is unaffected — it never
    routes through handle_ws."""
    ...
    blocked = _ScriptedWS([{"jsonrpc": "2.0", "id": 1, "method": "shell.exec", "params": {}}])
    asyncio.run(ws_module.handle_ws(blocked))
    assert "shell.exec" not in dispatched
    rejections = [m for m in blocked.sent if isinstance(m.get("error"), dict) and m["error"].get("code") == 4403]
    assert rejections and rejections[0]["id"] == 1

    allowed = _ScriptedWS(
        [{"jsonrpc": "2.0", "id": 2, "method": "prompt.submit", "params": {"session_id": "x", "text": "hi"}}]
    )
    asyncio.run(ws_module.handle_ws(allowed))
